...
public class DurationFormatUtils {
    ...
    public static String formatDurationWords(final long durationMillis, final boolean suppressLeadingZeroElements, final boolean suppressTrailingZeroElements) {
        String duration = formatDuration(durationMillis, "d' days 'H' hours 'm' "+ Main.getPlugin().getConfig().getString("OTHERS.COOLDOWN-FORMAT.MINUTES")+" 's' "+ Main.getPlugin().getConfig().getString("OTHERS.COOLDOWN-FORMAT.SECONDS")+"'");
        if (suppressLeadingZeroElements) {
            duration = " " + duration;
            String tmp = CooldownManager.replaceOnce(duration, " 0 days", "");
            if (tmp.length() != duration.length()) {
                duration = tmp;
                tmp = CooldownManager.replaceOnce(duration, " 0 hours", "");
                if (tmp.length() != duration.length()) {
                    duration = tmp;
                    tmp = (duration = CooldownManager.replaceOnce(duration, " 0 "+ Main.getPlugin().getConfig().getString("OTHERS.COOLDOWN-FORMAT.MINUTES"), ""));
                    if (tmp.length() != duration.length()) {
                        duration = CooldownManager.replaceOnce(tmp, " 0 "+ Main.getPlugin().getConfig().getString("OTHERS.COOLDOWN-FORMAT.SECONDS"), "");
                    }
                }
            }
        ...
        if (suppressTrailingZeroElements) {
            String tmp = CooldownManager.replaceOnce(duration, " 0 "+ Main.getPlugin().getConfig().getString("OTHERS.COOLDOWN-FORMAT.SECONDS"), "");
            if (tmp.length() != duration.length()) {
                duration = tmp;
                tmp = CooldownManager.replaceOnce(duration, " 0 "+ Main.getPlugin().getConfig().getString("OTHERS.COOLDOWN-FORMAT.MINUTES"), "");
                if (tmp.length() != duration.length()) {
                    duration = tmp;
                    tmp = CooldownManager.replaceOnce(duration, " 0 hours", "");
                    if (tmp.length() != duration.length()) {
                        duration = CooldownManager.replaceOnce(tmp, " 0 days", "");
                    }
                }
            }
        }
        duration = " " + duration;
        duration = CooldownManager.replaceOnce(duration, " 1 "+ Main.getPlugin().getConfig().getString("OTHERS.COOLDOWN-FORMAT.SECONDS"), " 1 "+ Main.getPlugin().getConfig().getString("OTHERS.COOLDOWN-FORMAT.SECOND"));
        duration = CooldownManager.replaceOnce(duration, " 1 "+ Main.getPlugin().getConfig().getString("OTHERS.COOLDOWN-FORMAT.MINUTES"), " 1 "+ Main.getPlugin().getConfig().getString("OTHERS.COOLDOWN-FORMAT.MINUTE"));
        duration = CooldownManager.replaceOnce(duration, " 1 hours", " 1 hour");
        duration = CooldownManager.replaceOnce(duration, " 1 days", " 1 day");
    ...
    static String format(final Token[] tokens, final int years, final int months, final int days, final int hours, final int minutes, final int seconds, int milliseconds, final boolean padWithZeros) {
        ...
        for (final Token token : tokens) {
            ...
            if (value instanceof StringBuffer) {
                ...
                buffer.append(padWithZeros ? CooldownManager.leftPad(Integer.toString(years), count, '0') : Integer.toString(years));
                lastOutputSeconds = false;
            }
            else if (value == DurationFormatUtils.M) {
                buffer.append(padWithZeros ? CooldownManager.leftPad(Integer.toString(months), count, '0') : Integer.toString(months));
                lastOutputSeconds = false;
            }
            else if (value == DurationFormatUtils.d) {
                buffer.append(padWithZeros ? CooldownManager.leftPad(Integer.toString(days), count, '0') : Integer.toString(days));
                lastOutputSeconds = false;
            }
            else if (value == DurationFormatUtils.H) {
                buffer.append(padWithZeros ? CooldownManager.leftPad(Integer.toString(hours), count, '0') : Integer.toString(hours));
                lastOutputSeconds = false;
            }
            else if (value == DurationFormatUtils.m) {
                buffer.append(padWithZeros ? CooldownManager.leftPad(Integer.toString(minutes), count, '0') : Integer.toString(minutes));
                lastOutputSeconds = false;
            }
            else if (value == DurationFormatUtils.s) {
                buffer.append(padWithZeros ? CooldownManager.leftPad(Integer.toString(seconds), count, '0') : Integer.toString(seconds));
                lastOutputSeconds = true;
            }
            else if (value == DurationFormatUtils.S) {
                if (lastOutputSeconds) {
                    milliseconds += 1000;
                    final String str = padWithZeros ? CooldownManager.leftPad(Integer.toString(milliseconds), count, '0') : Integer.toString(milliseconds);
                    buffer.append(str.substring(1));
                }
                else {
                    buffer.append(padWithZeros ? CooldownManager.leftPad(Integer.toString(milliseconds), count, '0') : Integer.toString(milliseconds));
                }
                lastOutputSeconds = false;
            }
        }
